...
This repo is a personal static website built with [Blume](https://useblume.dev/) 2 (an Astro-based static site generator). Source lives in `content/` (MDX articles), `pages/`, `blume.config.ts`, and `theme.css`. Standard commands are in `package.json` and `README.md`.
